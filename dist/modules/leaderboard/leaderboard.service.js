...
import { getPortalCountWithBestRecord as getPortalCountWithBestRecordRepo, getRecordsForLevel as getRecordsForLevelRepo, getRecordsForPlayer as getRecordsForPlayerRepo, submitRecord as submitRecordRepo } from './leaderboard.repository.js';
...
export const submitRecord = async (input) => {
...
export const getRecordsForLevel = async (query) => {
...
export const getPortalCountWithBestRecord = async (query) => {
    const recordsDirection = parseDirection(query.recordsDirection);
    ...
    const steamId = steamIdCandidate ?? undefined;
    const records = await getPortalCountWithBestRecordRepo(recordsDirection, portalDirection, recordsLimit, steamId);
    return { ok: true, data: { records } };
};
export const getRecordsForPlayer = async (query) => {
